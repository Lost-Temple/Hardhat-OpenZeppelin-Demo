...
function createOpcodes(opcodes) {
    ...
    for (const [key, value] of Object.entries(opcodes)) {
        ...
        result.set(code, new Opcode(Object.assign({ code, fullName: util_1.getFullname(code, value.name) }, value)));
    }
    return result;
}
/**
...
function getOpcodesForHF(common) {
    let opcodeBuilder = Object.assign({}, opcodes);
    for (let fork = 0; fork < hardforkOpcodes.length; fork++) {
        if (common.gteHardfork(hardforkOpcodes[fork].hardforkName)) {
            opcodeBuilder = Object.assign(Object.assign({}, opcodeBuilder), hardforkOpcodes[fork].opcodes);
        }
    }
    for (const eipOps of eipOpcodes) {
        if (common.isActivatedEIP(eipOps.eip)) {
            opcodeBuilder = Object.assign(Object.assign({}, opcodeBuilder), eipOps.opcodes);
        }
    }
