...
const debug = debug_1.debug('blockchain:clique');
...
class Blockchain {
    ...
    constructor(opts = {}) {
        ...
        this._validateConsensus = (_a = opts.validateConsensus) !== null && _a !== void 0 ? _a : true;
        this._validateBlocks = (_b = opts.validateBlocks) !== null && _b !== void 0 ? _b : true;
    ...
    async _init(genesisBlock) {
        let dbGenesisBlock;
        try {
            const genesisHash = await this.dbManager.numberToHash(new ethereumjs_util_1.BN(0));
        ...
        if (!genesisBlock) {
            ...
            genesisBlock = block_1.Block.genesis({}, { common });
        }
        ...
        if (!dbGenesisBlock) {
            ...
            const dbOps = [];
            dbOps.push(helpers_1.DBSetTD(genesisBlock.header.difficulty.clone(), new ethereumjs_util_1.BN(0), genesisHash));
            helpers_1.DBSetBlockOrHeader(genesisBlock).map((op) => dbOps.push(op));
            helpers_1.DBSaveLookups(genesisHash, new ethereumjs_util_1.BN(0)).map((op) => dbOps.push(op));
    ...
    async cliqueUpdateSignerStates(signerState) {
        ...
        const lastBlockNumber = (_a = blockSigners[blockSigners.length - 1]) === null || _a === void 0 ? void 0 : _a[0];
        if (lastBlockNumber) {
    ...
    async cliqueUpdateVotes(header) {
        ...
        if (header && !header.coinbase.isZero()) {
            ...
            for (let round = 1; round <= 2; round++) {
                ...
                const lastEpochBlockNumber = header.number.sub(header.number.mod(new ethereumjs_util_1.BN(this._common.consensusConfig().epoch)));
                const limit = this.cliqueSignerLimit();
                ...
                if (round === 1) {
                    ...
                    debug(`[Block ${header.number.toNumber()}] New clique vote: ${signer} -> ${beneficiary} ${nonce.equals(clique_1.CLIQUE_NONCE_AUTH) ? 'AUTH' : 'DROP'}`);
                }
                if (consensus) {
                    if (round === 1) {
        ...
        const lastBlockNumber = (_a = blockSigners[blockSigners.length - 1]) === null || _a === void 0 ? void 0 : _a[0];
        if (lastBlockNumber) {
            const lastEpochBlockNumber = lastBlockNumber.sub(lastBlockNumber.mod(new ethereumjs_util_1.BN(this._common.consensusConfig().epoch)));
    ...
    async getIteratorHead(name = 'vm') {
        return await this.initAndLock(async () => {
    ...
    async _putBlockOrHeader(item) {
        await this.runWithLock(async () => {
            ...
            const currentTd = { header: new ethereumjs_util_1.BN(0), block: new ethereumjs_util_1.BN(0) };
            let dbOps = [];
            ...
            let parentTd = new ethereumjs_util_1.BN(0);
            if (!block.isGenesis()) {
     ...
     */
    async setHead(tag, headHash) {
        await this.initAndLock(async () => {
    ...
    async _deleteCanonicalChainReferences(blockNumber, headHash, ops) {
        ...
        while (hash) {
            ...
            if (this._common.consensusAlgorithm() === 'clique') {
                ...
                this._cliqueLatestSignerStates = this._cliqueLatestSignerStates.filter((s) => !s[0].eq(blockNumber));
                await this.cliqueUpdateSignerStates();
                this._cliqueLatestVotes = this._cliqueLatestVotes.filter((v) => !v[0].eq(blockNumber));
                await this.cliqueUpdateVotes();
                this._cliqueLatestBlockSigners = this._cliqueLatestBlockSigners.filter((s) => !s[0].eq(blockNumber));
    ...
    async _rebuildCanonical(header, ops) {
        ...
        while (await loopCondition()) {
            ...
            const blockHash = header.hash();
            const blockNumber = header.number;
            helpers_1.DBSaveLookups(blockHash, blockNumber).map((op) => {
                ops.push(op);
            });
     ...
     * @hidden
     */
    async _getCanonicalHeader(number) {
